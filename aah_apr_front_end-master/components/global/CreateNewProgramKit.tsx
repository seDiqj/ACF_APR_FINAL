...
import { KitDatabaseProgram } from "@/types/Types";
...
const ProgramKitForm: React.FC<KitDatabaseProgramFormInterface> = ({
  ...
  useEffect(() => {
    ...
    Promise.all([
      requestHandler().get("/global/districts"),
      requestHandler().get("/projects/p/kit_database"),
    ])
      .then(([distRes, projRes]) => {
        setDistricts(Object.values(distRes.data.data));
        setProjects(Object.values(projRes.data.data));
      })
      .catch((error: any) => {
        reqForToastAndSetMessage(
          error.response?.data?.message || "Failed initializing parameters",
          "error"
        );
      })
      .finally(() => {
        if (!programId || IsCreateMode(mode)) setIsDataFetching(false);
      });
  }, [open]);
  ...
  useEffect(() => {
    ...
    requestHandler()
      ...
      .catch((error: AxiosError<any>) =>
        reqForToastAndSetMessage(
          error.response?.data?.message || "Failed parsing provinces",
          "error"
        )
      );
  ...
  useEffect(() => {
    if ((IsEditMode(mode) || IsShowMode(mode)) && programId && open) {
      ...
      requestHandler()
        ...
        .catch((error: any) =>
          reqForToastAndSetMessage(
            error.response?.data?.message || "Failed retrieving dataset",
            "error"
          )
        )
        .finally(() => setIsDataFetching(false));
    } else if (IsCreateMode(mode) && open) {
      setFormData(KitDatabaseProgramDefault());
      setFormErrors({});
    }
  }, [mode, programId, open]);
      ...
      <DialogContent className="max-w-4xl w-[90vw] max-h-[85vh] flex flex-col p-6 bg-card border border-border text-card-foreground shadow-2xl rounded-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <DialogHeader className="border-b border-border pb-4">
          <DialogTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            {IsCreateMode(mode) && "Create New Kit Distribution Program"}
            {IsEditMode(mode) && "Modify Kit Program Metrics"}
            {IsShowMode(mode) && "Kit Distribution Specifications"}
          </DialogTitle>
        </DialogHeader>
